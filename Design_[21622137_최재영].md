...
sequenceDiagram
    ...
    alt 고객 웹 로그인 (이메일 + 비밀번호)
        ...
        alt 인증 실패
            ...
            Login-->>main: 4: SUCCESS → 고객 메인화면 이동
        end
        deactivate Login
        ...
        Login-->>main: 3: SUCCESS → 접수 방식 선택 화면 이동
        deactivate Login
...
위의 그림은 시스템 실행 후 로그인 기능을 수행할 때를 표현한 Sequence Diagram이다. 사용자 유형에 따라 세 가지 로그인 경로가 존재한다. 고객·행원·관리자는 이메일+비밀번호(BCrypt 검증)를 사용하며, 키오스크는 주민번호(AES 암호화 후 비교)를 사용한다.
...
sequenceDiagram
    ...
    kiosk->>RF: 1: POST /py/auto-insert-task<br/>(task_detail_type 포함 — 고객이 직접 선택한 업무)
    activate RF
    RF->>RF: 2: 중복 접수 확인
    RF->>RF: 3: task_detail_type으로 창구 유형(A/B/C) 및<br/>assigned_level 결정 (RF 예측 없이 DETAIL_TYPE_META 참조)
    RF->>RF: 4: issueTicket — SELECT FOR UPDATE 잠금으로 번호 채번
    RF->>RF: 5: 처리 가능한 행원 배정
    RF-->>kiosk: 6: 번호표 정보 반환 (is_ai = false)
    deactivate RF
```

위의 그림은 고객이 키오스크에서 업무를 직접 선택하여 번호표를 발급받는 과정을 표현한 Sequence Diagram이다. 자동 접수와 달리 RF 모델 예측 없이 고객이 선택한 `task_detail_type`을 `DETAIL_TYPE_META`에서 직접 참조하여 창구 유형과 처리 직급을 결정한다. `is_ai = false`로 저장된다.
...
sequenceDiagram
    ...
    main->>CB: 1: POST /py/chat (message)<br/>(프론트엔드가 직접 호출)
    activate CB
    CB->>CB: 2: DB에서 활성 금융 상품 목록 조회
    CB->>CB: 3: site_guide + 상품 목록으로<br/>RAG 컨텍스트 구성
    alt 일일 한도 초과 (30회)
        CB-->>main: 4: 한도 초과 응답
    else 한도 내
        CB->>CB: 5: Gemini API 호출 및 응답 생성
        CB-->>main: 6: 챗봇 응답 반환
    end
    deactivate CB
```

위의 그림은 고객이 AI 챗봇 상담을 이용하는 과정을 표현한 Sequence Diagram이다. 프론트엔드가 Spring Boot를 거치지 않고 AI 서버(`/py/chat`)를 직접 호출한다. AI 서버는 DB에서 금융 상품 목록을 조회하고 site_guide.txt와 결합하여 RAG 컨텍스트를 구성한 뒤, 일일 한도(30회) 확인 후 Gemini API를 호출하여 응답을 프론트엔드에 직접 반환한다.
