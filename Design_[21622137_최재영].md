...
- 개인정보 보호를 위해 주민등록번호는 AES로 암호화하여 저장하되 암호화 키는 소스 코드가 아닌 외부 설정에서 주입하며, 화면 응답 시에는 평문 전체를 내려주지 않고 뒷자리를 마스킹한다. 고객 식별이 필요한 내부 처리(예: 법인 전환)는 주민번호를 클라이언트와 왕복시키지 않고 userId 기반으로 수행한다.
...
위의 그림은 시스템 실행 후 로그인 기능을 수행할 때를 표현한 Sequence Diagram이다. 사용자 유형에 따라 세 가지 로그인 경로가 존재한다. 고객·행원·관리자는 이메일+비밀번호(BCrypt 검증)를 사용하며 성공 시 서버 세션을 생성한다. 반면 키오스크는 주민번호(AES 암호화 후 비교)로 인증하되 웹 세션을 생성하지 않는 세션리스 방식으로, 인증 성공 시 userId/userName만 반환하여 이후 번호표 접수 요청에 userId를 직접 전달한다. 이를 통해 키오스크와 웹의 세션이 완전히 독립되어, 주민등록번호만으로 인증된 키오스크 흐름이 웹 전용 기능(비밀번호 변경·대출 신청·카드 발급 등)에 접근할 수 없고 동시 사용 시에도 상호 간섭하지 않는다.
