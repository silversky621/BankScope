...
sequenceDiagram
    ...
    activate AI
    AI->>AI: 3: DB에서 5개 피처 직접 조회<br/>(age, is_corporate, total_balance,<br/>has_active_loan, recent_tx_count)
    AI->>AI: 4: MinMaxScaler 정규화 + cosineSimilarity 계산
    AI->>AI: 5: 유사 고객 Top 20 선정 → 상위 상품 Top 3 product_id 추출
    AI->>AI: 6: 고객 나이 기준 가입 연령 조건 필터링<br/>(min_age / max_age)
    AI-->>SpringBoot: 7: 조건에 맞는 상품 상세 정보 목록 반환 (최대 Top 3)
    deactivate AI
    SpringBoot-->>main: 8: 추천 상품 화면 렌더링
    deactivate SpringBoot
```

위의 그림은 고객 로그인 직후 AI 금융 상품 추천이 수행될 때를 표현한 Sequence Diagram이다. Spring Boot는 `user_id`만 전달하며, AI 서버가 DB에서 직접 5개 피처를 조회한다. Cosine Similarity로 유사 고객 Top 20을 선정하여 상위 3개 product_id를 추출하고, 고객 나이의 가입 연령 조건(min_age/max_age) 필터링을 적용한 뒤 상품 상세 정보를 포함한 최종 목록을 Spring Boot에 반환한다. Spring Boot는 이를 그대로 메인화면에 렌더링한다.
