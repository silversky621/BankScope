...
시스템을 이용하기 위한 권한을 부여하는 클래스이다. 고객(user_type=0)으로 회원가입하며, 비밀번호는 BCrypt로 해싱하고 주민번호는 AES로 암호화하여 DB에 저장한다. AES 키는 소스 코드에 두지 않고 외부 설정에서 주입하여 저장소 노출을 차단하며, 저장된 주민번호를 행원 워크스페이스 등 화면에 표시할 때는 평문 전체를 내려주지 않고 뒷자리를 마스킹(`900101-1******`)하여 응답한다. 행원(member)은 별도 테이블에 직급(Level 1~5)과 함께 등록된다. 이 클래스를 통해 회원가입한 사용자만 로그인 기능을 사용할 수 있다.
...
| AES 암호화 | Advanced Encryption Standard. 주민등록번호를 DB에 저장할 때 적용하는 양방향 대칭키 암호화 방식. 비밀번호에는 사용하지 않는다. 암호화 키는 소스에 하드코딩하지 않고 외부 설정에서 주입하며, 화면 표시 시에는 복호화한 평문을 마스킹(뒷자리 가림)하여 노출을 최소화한다. |
