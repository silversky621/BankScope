...
| Success Post Condition | 서버 세션에 사용자 정보와 접속 채널 구분자(loginType: web 또는 kiosk)가 저장되고, 권한에 따라 고객 메인화면 / 행원 워크스페이스 / 관리자 대시보드로 라우팅된다. |
...
| 4 | 인증 성공 시 서버 세션에 사용자 정보와 loginType(웹 로그인: `web`, 키오스크 로그인: `kiosk`)을 저장한다. |
...
시스템을 사용하기 위해 실행해야 하는 클래스이다. 입력된 비밀번호를 BCrypt로 검증하여 DB의 해시값과 비교하고, 성공 시 서버 세션에 사용자 정보와 접속 채널 구분자(`loginType`)를 함께 저장한다. 웹 로그인은 `loginType=web`, 키오스크 로그인은 `loginType=kiosk`로 구분되며, 비밀번호 변경·대출 신청·카드 발급 등 민감한 작업은 `web` 타입 세션에서만 허용된다. 권한 코드에 따라 고객 메인화면, 행원 워크스페이스, 관리자 대시보드로 각각 라우팅한다.
