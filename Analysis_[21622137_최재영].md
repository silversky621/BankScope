...
| 4 | System은 사이트 이용 가이드 텍스트와 DB의 활성 금융 상품 목록을 컨텍스트로 구성한다. |
...
로그인한 회원 전용 금융 상담 클래스이다. 프론트엔드가 `/py/chat` 호출 시 SESSION 쿠키를 함께 전송하며, AI 서버는 이를 Redis에서 조회하여 user_id를 추출한 뒤 미인증 요청을 즉시 차단한다. 인증 성공 시 Google Gemini 2.5 Flash API를 호출하며, 사이트 이용 가이드 텍스트와 DB에서 실시간 조회한 금융 상품 목록을 컨텍스트로 함께 전달하는 RAG(Retrieval-Augmented Generation) 방식을 채택한다. 로그인 회원당 1일 30건의 이용 한도를 관리한다.
