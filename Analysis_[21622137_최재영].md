...
| 2026.06.02 | 2.5.0 | Redis 세션 저장소 도입 반영 (Use Case #7 챗봇 인증 단계 추가, Glossary Redis 항목 추가) | 최재영 |
| 2026.06.03 | 2.6.0 | 채널별 세션 타입(loginType) 분리 설계 반영 (Use Case #2 Login, Domain Analysis #3) | 최재영 |
| 2026.06.03 | 2.7.0 | 키오스크 세션리스 전환 반영 (웹 세션과 분리, userId 기반 접수) | 최재영 |
| 2026.06.04 | 2.8.0 | AI 챗봇 개인화(Use Case #9 의도 기반 본인 데이터 조회 단계 추가, ChatBot 클래스) 및 개인정보 보호(주민번호 마스킹·AES 키 외부화) 반영 | 최재영 |
| 2026.06.04 | 2.9.0 | 보안 고도화 반영: 주민번호 AES-GCM+HMAC 블라인드 인덱스, 서버측 역할/소유권 인가, Flyway 마이그레이션, Glossary 갱신 | 최재영 |
| 2026.06.09 | 2.10.0 | 관리자 대시보드 시간대별 예상 혼잡도(최근 8주 동일 요일 업무량을 가용 창구 수 대비로 환산) 반영: Use Case #10 및 대시보드 와이어프레임 정정 | 최재영 |
...
flowchart TB
    ...
    subgraph BankScope["BankScope"]
        ...
        %% 로그인은 각 기능의 사전 조건(공통 포함) → 기능이 Login을 «include»
        RF -.->|"«include»"| LG
        IP -.->|"«include»"| LG
        AC -.->|"«include»"| LG
        MQ -.->|"«include»"| LG
        MS -.->|"«include»"| LG
        %% 회원가입은 로그인 화면에서 선택적으로 분기되는 확장
        RM -.->|"«extend»"| LG
        %% 업무 이관은 대기열 관리 중 조건부로 발생하는 확장
        TT -.->|"«extend»"| MQ
    end
...
| Success Post Condition | 회원가입이 완료되어 BCrypt 해시된 비밀번호, AES-GCM으로 암호화된 주민번호, 검색용 HMAC 블라인드 인덱스가 DB에 저장된다. |
...
| 5 | System은 비밀번호를 BCrypt로 해싱하고, 주민번호는 AES-GCM으로 암호화(+ 검색용 HMAC 블라인드 인덱스 생성)하여 DB에 저장한다. |
| 6 | 회원가입이 성공하면 끝난다. |
| **EXTENSION SCENARIOS** | |
...
| Success Post Condition | 웹 로그인은 서버 세션에 사용자 정보가 저장되고 권한에 따라 고객 메인화면 / 행원 워크스페이스 / 관리자 대시보드로 라우팅된다. 키오스크 로그인은 세션을 생성하지 않고 userId를 반환하여 번호표 접수에 사용한다. |
...
| 4 | 웹 로그인은 인증 성공 시 서버 세션에 사용자 정보를 저장한다. (키오스크 로그인은 세션을 생성하지 않고 userId만 반환하는 세션리스 방식) |
...
|---|---|
| **GENERAL CHARACTERISTICS** | |
| Summary | 고객이 금융 상품 및 서비스 이용, 또는 본인의 금융 현황(잔액·대출·카드·가입 상품)에 관해 AI 챗봇에게 실시간 상담을 요청한다. |
| Scope | BankScope |
| Level | User Level |
| Author | 최재영 |
...
| 1 | 프론트엔드가 SESSION 쿠키를 첨부하여 AI 서버(`POST /py/chat`)를 직접 호출한다. |
| 2 | AI 서버는 SESSION 쿠키를 Redis에서 조회하여 user_id를 추출한다. |
| 3 | System은 1일 이용 한도(30건) 초과 여부를 확인한다. |
| 4 | System은 사이트 이용 가이드 텍스트와 DB의 활성 금융 상품 목록을 기본 컨텍스트로 구성한다. |
| 4-1 | System은 질문의 의도(잔액·대출·카드·가입 상품·추천)를 키워드로 감지하고, 해당 의도가 있을 때만 검증된 user_id로 본인의 금융 데이터를 추가 조회하여 컨텍스트에 결합한다. 이때 주민번호·계좌 비밀번호·카드 전체 번호·CVC 등 민감정보는 조회·결합에서 제외하며, 계좌·카드 번호는 뒤 4자리만 노출한다. |
| 5 | System은 구성된 컨텍스트와 고객 질문을 합쳐 Gemini 2.5 Flash API로 전송한다. |
| 6 | Gemini 모델은 컨텍스트를 참조하여 답변을 생성하고 반환한다. |
| 7 | 답변이 챗봇 화면에 출력되면 끝난다. |
| **EXTENSION SCENARIOS** | |
| Step | Branching Action |
| 2 | 2a. Redis에서 세션을 찾을 수 없는 경우 (미인증)<br>　2a.1. '챗봇 서비스는 로그인 후 이용 가능합니다' 메시지를 반환하고 종료한다. |
| 3 | 3a. 1일 이용 한도(30건)를 초과한 경우<br>　3a.1. '오늘 채팅 가능 횟수를 모두 사용하셨습니다' 메시지를 반환하고 종료한다. |
...
| 1 | System은 실시간 대기 현황(전체 대기 인원·운영 중 창구·오늘 총 처리 건수), 실시간 창구 모니터링, 시간대별 예상 혼잡도(최근 8주 동일 요일 이력을 현재 가용 창구 수 대비로 환산) 등을 대시보드에 표시한다. |
| 2 | 관리자는 금융 상품 등록/수정/삭제(가입 연령 조건 min_age/max_age 포함), 금리 변경, 회원 조회, 게시판 관리 등의 작업을 선택한다. |
...
시스템을 이용하기 위한 권한을 부여하는 클래스이다. 고객(user_type=0)으로 회원가입하며, 비밀번호는 BCrypt로 해싱하고 주민번호는 **AES-GCM**(랜덤 IV로 매번 다른 암호문)으로 저장한다. 주민번호 동등 검색(중복 확인·키오스크 로그인)은 비결정적 암호문 대신 **HMAC 블라인드 인덱스**(별도 컬럼)로 수행하여, DB 유출 시에도 암호문·인덱스에서 원문이 드러나지 않게 한다. 암복호화·인덱스 키는 소스에 두지 않고 외부 설정에서 주입하며(시드 주민번호는 기동 시 Flyway 마이그레이션이 그 환경의 키로 재암호화·인덱싱), 화면 표시 시에는 평문 전체 대신 뒷자리를 마스킹(`900101-1******`)한다. 행원(member)은 별도 테이블에 직급(Level 1~5)과 함께 등록된다. 이 클래스를 통해 회원가입한 사용자만 로그인 기능을 사용할 수 있다.

### 3) Login

시스템을 사용하기 위해 실행해야 하는 클래스이다. 웹 로그인은 입력된 비밀번호를 BCrypt로 검증하여 DB의 해시값과 비교하고, 성공 시 서버 세션에 사용자 정보를 저장한 뒤 권한 코드에 따라 고객 메인화면, 행원 워크스페이스, 관리자 대시보드로 각각 라우팅한다. 반면 키오스크 로그인(주민등록번호)은 웹 세션을 생성하지 않는 세션리스 방식으로, 인증 성공 시 userId만 반환하여 번호표 접수 요청에 활용한다. 이를 통해 키오스크와 웹의 세션이 완전히 독립되며, 주민등록번호만으로 인증된 키오스크 흐름이 웹 전용 기능(비밀번호 변경·대출 신청·카드 발급 등)에 접근할 수 없다.
...
- **SHAP Explainer:** Random Forest 예측 결과에 대한 피처 기여도를 분석. 개발·검증용 엔드포인트(`GET /py/explain/{user_id}`)는 Waterfall 차트 이미지로 시각화하고, 은행원용 엔드포인트(`GET /py/explain/{user_id}/summary`)는 기여도 상위 피처를 자연어 상담 근거로 변환하여 워크스페이스에 제공(최종 판단은 상담원이 수행한다는 면책 표기 포함).

CSV 가상 데이터와 실제 DB 데이터를 서버 시작 시 자동 병합하여 추천 모델을 구성하며, RF 모델은 `bank_model.pkl`로 저장·로드된다.

### 9) ChatBot

로그인한 회원 전용 금융 상담 클래스이다. 프론트엔드가 `/py/chat` 호출 시 SESSION 쿠키를 함께 전송하며, AI 서버는 이를 Redis에서 조회하여 user_id를 추출한 뒤 미인증 요청을 즉시 차단한다. 인증 성공 시 Google Gemini 2.5 Flash API를 호출하며, 사이트 이용 가이드 텍스트와 DB에서 실시간 조회한 금융 상품 목록을 컨텍스트로 함께 전달하는 RAG(Retrieval-Augmented Generation) 방식을 채택한다. 여기에 더해, 질문의 의도를 감지(`_detect_intents`)하여 잔액·대출·카드·가입 상품 등 해당 의도에 필요한 본인 데이터만 검증된 user_id로 선택적으로 조회(`_load_user_context`)해 컨텍스트에 결합하는 개인화 기능을 제공한다. 이는 실무의 function-calling 패턴을 경량화한 것으로, 불필요한 데이터 조회를 피해 응답 정확도·비용·감사 추적성을 확보한다. 주민번호·계좌 비밀번호·카드 전체 번호·CVC 등 민감정보는 조회 단계에서 제외하고 계좌·카드 번호는 뒤 4자리만 노출하며, 프롬프트 가드레일로도 민감정보 노출을 금지한다. 로그인 회원당 1일 30건의 이용 한도를 관리한다. (본 PoC는 공개 LLM API를 사용하나, 실서비스에서는 금융 규제상 프라이빗·온프레미스 LLM과 비식별화가 필요하다.)
...
시스템 전반을 관리하는 클래스이다. 금융 상품 등록·수정·삭제(상품명, 금리, 가입 기간·금액, 가입 연령 조건(min_age/max_age) 등), 금리 관리, 회원 조회·관리, 게시판 관리, 창구 대기열 강제 이관 기능을 제공한다. 관리자가 설정한 가입 연령 조건은 AI 상품 추천 시 필터링 기준으로 활용된다. 관리자 대시보드에서 실시간 대기 현황, 업무 유형별 처리 비율(Level 1~5), 시간대별 방문 통계를 모니터링할 수 있다.
...
=================================================================
  실시간 현황 (KPI)
  ┌──────────────┬──────────────┬───────────────┬──────────────┐
  │ 전체 대기 인원│ 운영중 창구  │ 오늘 총 처리 건│ 전 창구 공지 │
  │    12명       │    5개       │    87건        │ [입력][전송] │
  └──────────────┴──────────────┴───────────────┴──────────────┘
  ('전체 대기 인원' 클릭 → 전체 대기열 + 창구 강제 이관 모달)

  ┌─ 실시간 창구 모니터링 ──────┐  ┌─ 시간대별 예상 혼잡도 ───────┐
  │ 창구1  김행원 Lv.2  처리중  │  │  9 10 11 12 13 14 15 16 17시 │
  │ 창구2  이행원 Lv.4  대기    │  │  ▁  ▃  █  ▅  ▂  ▄  █  ▆  ▃  │
  │ ...                         │  │ (최근 8주 동일 요일 업무량을  │
  │                             │  │  현재 가용 창구 수 대비 %로)  │
  └─────────────────────────────┘  └──────────────────────────────┘
...
| Gemini 2.5 Flash | Google의 경량 LLM 모델. 사이트 가이드·금융 상품 정보의 RAG에 더해, 질문 의도에 따라 로그인 고객 본인의 금융 데이터를 선택적으로 결합하여 개인화 챗봇 상담에 활용. |
...
| AES-GCM | 주민등록번호 저장에 쓰는 인증된 대칭키 암호화 모드. 매번 다른 IV를 써서 같은 평문도 암호문이 달라지고 변조 검출을 제공한다. 키는 소스가 아닌 외부 설정에서 주입하고, 화면 표시 시에는 복호화한 평문을 마스킹한다. 비밀번호에는 사용하지 않는다(BCrypt). |
| HMAC 블라인드 인덱스 | 평문을 비밀키로 HMAC-SHA256 해시한 결정적·단방향 값. AES-GCM 암호문이 비결정적이라 직접 검색이 불가하므로, 주민번호 동등 검색(중복 확인·키오스크 로그인)을 위해 별도 컬럼에 저장한다. 값만으로는 원문 복원 불가. |
| Flyway | DB 스키마·시드 데이터를 버전(V1, V2…)으로 관리·자동 적용·추적하는 마이그레이션 도구. 기동 시 적용되며, 주민번호 시드는 환경별 키로 재암호화하는 마이그레이션을 둔다. |
| Redis | 인메모리 Key-Value 저장소. Spring Boot와 FastAPI AI 서버 간의 공유 세션 저장소로 활용된다. 로그인 시 세션 ID와 user_id 매핑을 저장하고 AI 서버가 챗봇 인증에 사용한다. |
| SESSION 쿠키 | Spring Session이 발급하는 세션 식별자 쿠키. 브라우저가 동일 도메인 요청에 자동 첨부하며, AI 서버는 이를 Redis에서 조회하여 user_id를 추출한다. |

---

## 6. References

- BankScope Conceptualization Document (v2.9.0, 2026.06.04)
