...
| 26/06/04 | 2.9.0 | 보안 고도화 반영: 주민번호 AES-GCM+HMAC 블라인드 인덱스 전환, 서버측 역할/소유권 인가, DB 마이그레이션(Flyway) 관리, 한계 및 향후 개선 절 추가 | 최재영 |
| 26/06/09 | 2.10.0 | 관리자 대시보드 시간대별 예상 혼잡도(최근 8주 동일 요일 업무량을 가용 창구 수 대비로 환산) 모니터링 반영 (Manage System) | 최재영 |
...
|---|---|
| **Actor** | Admin |
| **Description** | 관리자는 금융 상품 등록/수정/삭제(상품명·금리·가입 기간·금액·가입 연령 조건(min_age/max_age) 등), 금리 관리, 회원 조회/관리, 게시판 관리, 창구 대기열 강제 이관 등 시스템 전반을 관리한다. 관리자가 설정한 가입 연령 조건은 AI 상품 추천의 필터링 기준으로 활용된다. 관리자 대시보드에서는 실시간 대기 현황·창구 운영 상태와 함께, 최근 8주 동일 요일 이력을 현재 가용 창구 수 대비로 환산한 시간대별 예상 혼잡도를 모니터링할 수 있다. |
...
| **Approach** | 회원가입 시 비밀번호는 BCrypt 단방향 해싱하여 저장하고, 주민등록번호는 **AES-GCM**(매번 다른 IV로 암호화되어 같은 평문도 암호문이 달라짐)으로 저장한다. 동등 검색이 필요한 경우 평문/암호문 대신 **HMAC 기반 블라인드 인덱스**(결정적·단방향)로 조회하여, DB가 유출되어도 암호문·인덱스에서 원문이 드러나지 않게 한다. 암호화·인덱스 키는 소스에 두지 않고 외부 설정에서 주입하며, 시드 데이터는 기동 시 **DB 마이그레이션(Flyway)** 이 그 환경의 키로 재암호화·인덱싱한다. 화면 표시 시에는 평문 전체 대신 뒷자리를 마스킹(예: `900101-1******`)한다. 웹 로그인(이메일+비밀번호) 성공 시 서버 세션에 사용자 정보를 저장하고 권한(user_type)에 따라 라우팅하며, **민감 API(거래·리스크·법인 처리·행원 대시보드 등)는 프론트 라우팅이 아니라 서버에서 세션 기반 역할/소유권을 검증**한다. 반면 키오스크 로그인(주민등록번호)은 별도의 웹 세션을 생성하지 않는 세션리스(stateless) 방식으로, 인증 성공 시 userId만 반환하여 이후 번호표 접수 요청에 userId를 직접 전달한다. 이를 통해 키오스크 흐름이 웹 세션·웹 전용 기능(비밀번호 변경·대출 신청·카드 발급 등)에 접근하는 것을 원천 차단하며, 온라인 키오스크 접수와 웹 브라우징을 동시에 사용해도 상호 간섭이 없다. |
| **Dynamics** | 서비스를 처음 이용하거나 기존 계정으로 로그인하는 경우. |
| **Goals** | BCrypt 비밀번호 인증, AES-GCM+HMAC 블라인드 인덱스 및 응답 마스킹을 통한 개인정보 보호, 서버측 역할/소유권 인가, Flyway 기반 스키마·시드 버전 관리, 키오스크 세션리스 설계를 통한 채널 간 세션 독립. |
...
AI가 특정 창구로 배정한 이유를 사용자나 관리자가 이해할 수 없으면 시스템 신뢰도가 낮아진다. (해결책: SHAP(SHapley Additive exPlanations) TreeExplainer를 적용하여 `GET /py/explain/{user_id}` 엔드포인트에서 특정 고객의 예측 근거를 Waterfall 차트 이미지로 반환하여 피처별 기여도를 시각화하고, 별도의 `GET /py/explain/{user_id}/summary` 엔드포인트는 기여도 상위 피처를 비개발자인 은행원이 바로 활용할 수 있는 자연어 상담 근거로 변환해 워크스페이스에 제공한다. 단, AI 결과는 참고 정보이며 최종 판단은 상담원이 수행한다는 면책을 함께 표기한다.)
...
| SHAP (SHapley Additive exPlanations) | AI 모델의 예측 결과에 대해 각 입력 피처가 얼마나 기여했는지를 수치로 설명하는 XAI(설명 가능한 AI) 기법. 본 시스템에서 창구 배정 근거를 개발·검증용 Waterfall 차트와 은행원용 자연어 상담 근거 양쪽으로 제공하는 데 활용된다. |
...
| AES-GCM | 인증된 암호화 모드. 매번 다른 IV를 사용해 같은 평문도 다른 암호문이 되며 변조 검출(tag)을 제공한다. 본 시스템에서 주민등록번호 저장에 사용한다. |
| HMAC 블라인드 인덱스 | 평문을 비밀키로 HMAC 해시한 결정적·단방향 값. 암호문(AES-GCM)이 비결정적이라 직접 검색이 불가하므로, 동등 검색(예: 키오스크 로그인)을 위해 별도 컬럼에 저장하는 검색용 인덱스. 값만으로는 원문 복원이 불가능하다. |
| Flyway | DB 스키마·시드 데이터를 버전(V1, V2…)으로 관리·자동 적용·추적하는 마이그레이션 도구. 본 시스템은 기동 시 마이그레이션을 자동 적용하며, 주민번호 시드는 그 환경의 키로 재암호화하는 마이그레이션을 둔다. |

---

## 7. 한계 및 향후 개선

- **암호화 키 관리:** 현재는 시드 호환을 위해 외부 설정(`application.properties`, 깃 제외) 기반 고정 키를 사용한다. 운영 환경에서는 시크릿 매니저(예: Vault, AWS Secrets Manager) 또는 환경 변수 주입으로 키를 분리하고 주기적 교체(rotation)·접근 감사를 적용하는 것이 바람직하다.
- **외부 LLM과 개인정보:** 챗봇은 공개 LLM(Gemini) API를 사용하므로, 실서비스에서는 금융 규제(망분리·개인정보보호)상 프라이빗·온프레미스 LLM과 비식별화, 그리고 LLM이 직접 데이터를 다루지 않는 function-calling 기반 사내 API 경유 구조가 요구된다.
- **테스트 커버리지:** 핵심 비즈니스 로직 일부에 단위 테스트를 두었으나, 인증·소유권·거래 무결성 전반에 대한 통합 테스트로 확대할 필요가 있다.
- **자동화 테스트/배포 파이프라인(CI/CD)** 및 부하·동시성 시나리오 검증은 후속 과제로 둔다.
