...
| 26/06/03 | 2.7.0 | 키오스크 세션리스 전환 반영 (웹 세션과 분리, 키오스크는 userId 기반 접수) | 최재영 |
| 26/06/04 | 2.8.0 | AI 챗봇 개인화(의도 기반 본인 금융데이터 조회·민감정보 제외) 및 개인정보 보호 강화(주민번호 응답 마스킹, AES 키 외부 설정 분리) 반영 | 최재영 |
...
**셋째, AI 금융 챗봇:** Google Gemini 2.5 Flash 모델을 기반으로, 사이트 이용 가이드와 현재 DB의 금융 상품 정보를 컨텍스트로 제공하여 고객 질문에 실시간 답변하는 AI 상담원을 제공한다. 나아가 질문의 의도를 감지하여 로그인 고객 본인의 금융 데이터(계좌 잔액, 대출 상환 일정, 보유 카드, 가입 상품)를 선택적으로 조회·결합함으로써 개인화된 답변을 제공하되, 주민등록번호·계좌 비밀번호·카드 전체 번호 등 민감정보는 컨텍스트에서 원천 제외한다.
...
|---|---|
| **Actor** | Customer |
| **Description** | 로그인한 회원이 챗봇 화면에서 질문을 입력하면 Gemini 2.5 Flash 모델이 사이트 이용 가이드와 현재 DB의 금융 상품 정보를 컨텍스트로 참조하여 답변한다. 또한 질문 의도에 따라 해당 회원 본인의 금융 데이터(잔액·대출·카드·가입 상품)를 선택적으로 조회하여 개인화된 답변을 제공하며, 민감정보(주민번호·계좌 비밀번호·카드 전체 번호 등)는 컨텍스트에서 제외한다. 로그인 회원당 1일 30건의 이용 한도가 적용된다. |
...
|---|---|
| **Actor** | Admin |
| **Description** | 관리자는 금융 상품 등록/수정/삭제(상품명·금리·가입 기간·금액·가입 연령 조건(min_age/max_age) 등), 금리 관리, 회원 조회/관리, 게시판 관리, 창구 대기열 강제 이관 등 시스템 전반을 관리한다. 관리자가 설정한 가입 연령 조건은 AI 상품 추천의 필터링 기준으로 활용된다. 관리자 대시보드에서는 실시간 대기 현황과 창구 운영 상태를 모니터링할 수 있다. |
...
| **Approach** | 회원가입 시 비밀번호는 BCrypt 단방향 해싱하여 DB에 저장하고, 주민등록번호는 AES 양방향 암호화하여 저장한다. 이때 AES 암호화 키는 소스 코드에 하드코딩하지 않고 외부 설정(환경 변수/`application.properties`)에서 주입하여 버전 관리 저장소에 노출되지 않도록 분리한다. 또한 저장된 주민등록번호를 화면에 표시할 때는 평문 전체를 반환하지 않고 생년월일·성별 자리를 제외한 뒷자리를 마스킹(예: `900101-1******`)하여, 평문이 서버 밖으로 나가지 않도록 한다. 웹 로그인(이메일+비밀번호) 성공 시 서버 세션에 사용자 정보를 저장하고 권한(user_type)에 따라 고객 메인화면, 행원 워크스페이스, 관리자 대시보드로 라우팅한다. 반면 키오스크 로그인(주민등록번호)은 별도의 웹 세션을 생성하지 않는 세션리스(stateless) 방식으로 동작하여, 인증 성공 시 userId만 반환하고 이후 번호표 접수 요청에 userId를 직접 전달한다. 이를 통해 주민등록번호만으로 인증된 키오스크 흐름이 웹 세션을 점유하거나 웹 전용 기능(비밀번호 변경·대출 신청·카드 발급 등)에 접근하는 것을 원천 차단하며, 온라인 키오스크 접수와 웹 브라우징을 동시에 사용해도 상호 간섭이 없다. |
| **Dynamics** | 서비스를 처음 이용하거나 기존 계정으로 로그인하는 경우. |
| **Goals** | BCrypt 해싱 기반의 안전한 비밀번호 인증, AES 키 외부화 및 주민번호 응답 마스킹을 통한 개인정보 보호, 역할(Role)별 기능 접근 제어, 키오스크 세션리스 설계를 통한 채널 간 세션 독립 및 권한 범위 분리. |
...
| **Approach** | 챗봇은 로그인한 회원 전용 서비스이다. 고객이 질문을 입력하면 프론트엔드가 AI 서버(`POST /py/chat`)를 직접 호출하며, 이 때 브라우저가 SESSION 쿠키를 자동으로 첨부한다. AI 서버는 SESSION 쿠키를 Redis에서 조회하여 user_id를 추출하고, 인증에 실패하면 즉시 거부한다. 인증 성공 시 사이트 이용 가이드 텍스트와 현재 DB에서 조회한 활성 금융 상품 목록을 기본 컨텍스트로 구성한다. 이에 더해 질문의 의도(잔액·대출·카드·가입 상품·상품 추천)를 키워드로 감지하여, **해당 의도에 필요한 데이터만** 검증된 user_id로 조회해 컨텍스트에 결합하는 의도 기반 선택적 조회(실무의 function-calling 패턴을 경량화한 방식)를 적용한다. 이렇게 구성한 프롬프트를 Google Gemini 2.5 Flash API에 전송하며, 주민등록번호·계좌 비밀번호·카드 전체 번호·CVC 등 민감정보는 조회 단계에서부터 제외하고 프롬프트 가드레일로도 노출을 금지한다. 참조 데이터에 없는 내용은 직접 방문 또는 고객센터 문의를 안내하도록 프롬프트가 설계되어 있다. 인증된 사용자당 1일 30건 한도로 제한된다. (본 PoC는 공개 LLM API를 사용하나, 실서비스에서는 금융 규제상 프라이빗·온프레미스 LLM과 비식별화 적용이 필요하다.) |
| **Dynamics** | 로그인한 고객이 금융 상품, 금리, 이용 방법, 또는 본인의 잔액·대출 상환일·보유 카드 등을 웹에서 즉시 문의하는 경우. |
| **Goals** | LLM 기반 실시간 금융 상담 및 본인 데이터 기반 개인화 응대로 고객 편의 향상 및 단순 문의 자동화. Redis 세션 인증으로 미인증 API 호출 차단, 의도 기반 최소 조회로 민감정보 노출 최소화. |
...
| Gemini 2.5 Flash | Google DeepMind의 경량 LLM 모델. 본 시스템에서는 사이트 가이드와 금융 상품 정보를 컨텍스트로 제공하는 RAG 방식에 더해, 질문 의도에 따라 로그인 고객 본인의 금융 데이터를 선택적으로 결합하여 개인화된 챗봇 상담에 활용된다. |
